...
--Ignore unit cap on capture
function TransferCaptureTarget(objective, captureTarget, army, captureThreshold)
    captureThreshold = captureThreshold or 0.98
    if not captureTarget or captureTarget.Dead then
        WARN('Missing captureTarget or captureTarget is dead.')
        return nil
    end

    return ForkThread(function()
        while (not objective or objective.Active) and not captureTarget.Dead do
            WaitTicks(1)

            if (captureTarget.captureProgress or 0) >= captureThreshold then
                local newUnit = GiveUnitToArmy(captureTarget, army, true)

                if objective then
                    if objective.ManualResult then
                        if objective.ManualResult(true)
                    else
                        objective.Active = false
                        objective:OnResult(true, {newUnit})
                    end
                end
